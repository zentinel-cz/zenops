...
import { AdminSubcontractorDailyWorkflow } from "@/components/SubcontractorDailyWorkflow";

export default function MowingListPage() {
  const [section, setSection] = useState<"records" | "daily">("records");
  const [dailySection, setDailySection] = useState<"ovecky" | "subcontractors">("ovecky");
  ...
  const sectionTabs = isAdmin && (
    <div className="flex flex-wrap gap-2 rounded-2xl border border-white/70 bg-white/75 p-2 shadow-sm">
      ...
      <button type="button" onClick={() => setSection("daily")} className={`rounded-xl px-4 py-2.5 text-sm font-bold transition-colors ${section === "daily" ? "bg-slate-900 text-white" : "text-slate-600 hover:bg-slate-100"}`}>Denní záznamy</button>
    </div>
  );
