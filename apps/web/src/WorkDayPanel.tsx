...
export function WorkDayPanel({ projects, assetVersion }: { projects: ProjectSummary[]; assetVersion: number }) {
  ...
  useEffect(() => {
    void fetch("/api/assets", { credentials: "include" }).then(async (response) => response.json() as Promise<{ machines: Machine[]; attachments: Attachment[] }>).then((body) => { setMachines(body.machines); setAttachments(body.attachments); }).catch(() => setError("Katalog prostředků se nepodařilo načíst."));
  }, [assetVersion]);
